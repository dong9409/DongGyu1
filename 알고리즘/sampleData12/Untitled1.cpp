...
void Diet(vector<food>& order){
	...
	if(promising == true){
		if(ans_price > price_total){
			ans = order;
			ans_price = price_total;
			ans_material = material_total;
			return;
		}
		else if(ans_price == price_total){
			if(ans_material < material_total){
				ans = order;
				ans_price = price_total;
				ans_material = material_total;
				return ;
			}
			else if(ans_material == material_total){
				ans = comp(ans, order);
				return ;
			}
		}
